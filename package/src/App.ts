...
export default {
    ...
    init(canvas, eventTarget) {
        let enabled = true
        const elementClassCache = new Map()
        ...
        const createClassToggle = (className: string, element: HTMLElement) => {
        ...
        const renderClassToggles = (element: HTMLElement) => {
        ...
        const elementIsPartOfTheDevToolbarWindow = (element: HTMLElement): boolean => {
        ...
        function listenForPageClicks(event: MouseEvent) {
            // Check for !enabled in case the listener wasn't removed correctly
            if (!enabled) {
                return
            }

            if (elementIsPartOfTheDevToolbarWindow(event.target as HTMLElement)) {
                return
            }
            ...
            const checkForClasses = (target: HTMLElement): boolean => {
            ...
            let elementHasClass = checkForClasses(element as HTMLElement) || elementClassCache.has(element)
